...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // deleteFile trashes a file in the script owner's Drive; listManagedFiles
    // enumerates it. Every other action (upload, uploadResearchFile, sync*,
    // email*, trigger*, health/ping) is unaffected and still works for all
    // roles. Fails closed if the service-role key is missing -- for these two
    // actions only. Mirrors the role check in api/team/remove-member.ts.
