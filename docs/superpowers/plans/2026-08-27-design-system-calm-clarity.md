...
- [ ] **Step 0: `src/index.css` cleanup** (two carried items):
  ...
  - **Carried from Task 9 review — strip 2 residual `uppercase`:** `src/components/calendar/MobileDateStripView.tsx:115` `{dayShort}` weekday header ("Mon"/"Tue") — remove ` uppercase` from its className (sibling `CalendarWeekView.tsx:113` already renders sentence-case). And `src/components/AuditLogView.tsx:259` `{event.entityType}` mono tag — remove ` uppercase` (adjacent `actionType` badge same row is already lowercased). Collapse any doubled space.
