  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 bg-white border border-[#bfcab4] rounded shadow-xs space-y-2">
          <span className="font-label-caps text-[10px] text-[#707a67] uppercase font-bold block">Posted</span>
          <div className="font-display-xl text-3xl font-bold text-[#296c00]">{postedCount}</div>
          <p className="font-code-sm text-[10px] text-[#296951]">Published to Instagram</p>
        </div>

        <div className="p-5 bg-white border border-[#bfcab4] rounded shadow-xs space-y-2">
          <span className="font-label-caps text-[10px] text-[#707a67] uppercase font-bold block">Ready to Post</span>
          <div className="font-display-xl text-3xl font-bold text-[#1b1c1a]">{readyToPostCount}</div>
          <p className="font-code-sm text-[10px] text-[#707a67]">Reminder Set</p>
        </div>

        <div className="p-5 bg-white border border-[#bfcab4] rounded shadow-xs space-y-2">
          <span className="font-label-caps text-[10px] text-[#707a67] uppercase font-bold block">In Progress</span>
        ...
        <div className="p-5 bg-white border border-[#bfcab4] rounded shadow-xs space-y-2">
          <span className="font-label-caps text-[10px] text-[#707a67] uppercase font-bold block">Ideas</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-[#bfcab4] p-6 rounded shadow-xs space-y-4">
          <h3 className="font-label-caps text-xs font-bold text-[#296c00] uppercase">Posts over the next 14 days</h3>

          <div className="h-44 bg-[#faf9f5] border border-[#bfcab4] rounded p-4 flex flex-col justify-between">
            {upcomingDays.every((d) => d.count === 0) ? (
              <div className="flex-1 flex items-center justify-center text-center">
                <p className="text-xs font-body-md text-[#707a67]">Nothing scheduled in the next two weeks yet.</p>
              </div>
            ) : (
              <div className="flex items-end gap-1.5 h-24 flex-1">
                {upcomingDays.map((day) => (
                  <div
                    ...
                    className="flex-1 bg-[#296c00] rounded-xs hover:bg-[#78d24b] transition-colors"
            ...
            <div className="flex justify-between items-center font-code-sm text-[10px] text-[#707a67]">
        ...
        <div className="bg-white border border-[#bfcab4] p-6 rounded shadow-xs space-y-4">
                ...
                <div key={post.id} className="p-3 bg-[#faf9f5] border border-[#bfcab4] rounded text-xs space-y-2">
                  <div className="flex justify-between items-start gap-2">
                    ...
                    {(post.scheduledDate || post.scheduledTime) ? (
                      <span className="flex-shrink-0 font-label-caps text-[9px] px-1.5 py-0.5 bg-[#efeeea] border border-[#bfcab4] text-[#707a67] rounded uppercase">
                        ⏰ {post.scheduledDate}{post.scheduledTime ? ` ${post.scheduledTime}` : ''}
                      </span>
                    ) : (
                      <span className="flex-shrink-0 font-label-caps text-[9px] px-1.5 py-0.5 bg-[#faf9f5] border border-[#bfcab4] text-[#bfcab4] rounded uppercase">
                        No Reminder
                      </span>
                    )}
                  </div>
                  {post.reminderEmail && (
                    <div className="flex items-center gap-1 font-code-sm text-[10px] text-[#707a67]">
                      <span className="material-symbols-outlined" style={{ fontSize: '13px' }}>mail</span>
                      <span className="truncate">{post.reminderEmail}</span>
                    </div>
                  )}
                  <p className="font-body-md text-[#404a39] line-clamp-1">{post.caption}</p>

                  <div className="flex items-center justify-end gap-2 pt-1 border-t border-[#bfcab4]/40">
                    {onSelectPost && (
                      <button
                        onClick={() => onSelectPost(post)}
                        className="font-label-caps text-[10px] text-[#296c00] font-bold hover:underline px-2 py-0.5 bg-[#efeeea] rounded"
                      >
                        Edit ✏️
                      </button>
                    )}
                    {onDeletePost && (
                      <button
                        ...
                        className="font-label-caps text-[10px] text-[#ba1a1a] font-bold hover:underline px-2 py-0.5 bg-[#ffdad6] rounded"
                      >
                        Delete 🗑️
                      </button>
                    )}
      ...
      <div className="bg-white border border-[#bfcab4] rounded shadow-xs">
        <div className="p-5 border-b border-[#bfcab4] flex items-center justify-between">
          <div>
            <h3 className="font-label-caps text-xs font-bold text-[#296c00] uppercase">Who Did the Most Work</h3>
            <p className="font-body-md text-xs text-[#707a67] mt-0.5">Posts assigned and completed per team member</p>
          </div>
          <span className="material-symbols-outlined text-[#296c00]">leaderboard</span>
        </div>

        {teamMembers.length === 0 ? (
          <div className="p-8 text-center">
            <span className="material-symbols-outlined text-3xl text-[#bfcab4] block mb-2">group</span>
            <p className="font-label-caps text-[10px] text-[#707a67] uppercase">No team members added yet</p>
            <p className="font-body-md text-xs text-[#707a67] mt-1">Go to Settings → Team to add your team.</p>
          </div>
        ) : (
            ...
            {(() => {
              ...
              return unassignedCount > 0 ? (
                <div className="p-4 flex items-center gap-4">
                  ...
                  <div className="w-10 h-10 rounded-full bg-[#efeeea] border border-[#bfcab4] flex items-center justify-center flex-shrink-0">
                    <span className="material-symbols-outlined text-[#707a67] text-lg">person</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-body-md text-sm font-bold text-[#707a67] truncate">Other / Unassigned</p>
                    <p className="font-label-caps text-[9px] text-[#bfcab4] uppercase">Not in current team list</p>
                  </div>
                  <div className="text-center flex-shrink-0">
                    <div className="font-display-xl text-lg font-bold text-[#707a67]">{unassignedCount}</div>
